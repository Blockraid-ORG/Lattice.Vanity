...
app.use(cors({ origin: ['http://localhost:3000', process.env.CLIENT_ORIGIN].filter(Boolean) }));
...
app.post("/predict", verifySigner, async (req, res) => {
  try {
    ...
    console.log(`🚀 Mulai mencari suffix "${suffix}" dengan ${cpuCount} threads...`);
      ...
      for (let i = 0; i < cpuCount; i++) {
        ...
        worker.on("message", (msg) => {
          if (msg.progress !== undefined) {
            process.stdout.write(`Thread ${msg.thread}: ${msg.progress}%\r`);
          }

          if (msg.found && !found) {
            found = msg;
            console.log(`\n🎯 Ditemukan! ${msg.address} dengan salt ${msg.salt}`);
